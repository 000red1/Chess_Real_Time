...
public class ChessModel implements ActionListener{
  ...
  public void actionPerformed(ActionEvent evt){
    if(evt.getSource() == ssm){
      ...
      //Case 0a: Connection Test - Server 
      if(strSplit[0].equals("WHITE")){
        ssm.sendText("BLACK");
        //Case 0: Connection Test - Client 
      }else if(strSplit[0].equals("BLACK")){
        blnConnect = true;
        
      //Case 1: Game Start
      }else if(strSplit[0].equals("0")){
        startGame();
      ...
      //Case 1: Move Piece
        ...
        pieceArray[intX][intY].intOldX = intX1;
        pieceArray[intX][intY].intOldY = intY1;
      ...
      }else if(strSplit[0].equals("9")){
        blnGame=true;
        System.out.println("Checkmate! from server");
        pieceArray = new Piece[8][8];
        blnWinner=false;
        System.out.println(blnWinner);
      }
    } 
  }
  ...
  public void sendMessage(int intMessageNumber){
    String strMessage="2,"+strCurrentColour+","+strQuickChatArray[intMessageNumber];
    ssm.sendText(strMessage);
    strCurrentMessage = strCurrentColour+": "+strQuickChatArray[intMessageNumber];
  }
   ...
   * @see connect
   */
  public void join(String strIP, String strColour){
    ssm = new SuperSocketMaster(strIP,6112,this);
    ssm.connect();
    ssm.sendText(strColour);
  }
  ...
  public void move(int intX, int intY){
    //Case 0: Checks if the current selected piece is occupied by an enemy piece.
    try{
      if(pieceArray[intX1Selected][intY1Selected].strColour.equals("BLACK")){
    ...
    //Case 2: Highlighted square.
      ...
      pieceArray[intX][intY].intOldX = intX1Selected;
      pieceArray[intX][intY].intOldY = intY1Selected;
      
      //2. Sends the network message
      String strsend="1,"+intX+","+intY+","+intX1Selected+","+intY1Selected; 
      ssm.sendText(strsend);
      
      //3. Scan board for check and pawn promotions
      promotedPawn();
      checkKing();
      
      //4. Change the piece's max timer and starts the cooldown timer thread.
      pieceArray[intX][intY].intMaxCount = globalTimer.intPieceMaxTimer;
      Thread t1 = new Thread (pieceArray[intX][intY]);
      t1.start();
      
      //5. Disable the blnNotMoved variable.
      pieceArray[intX][intY].blnNotMoved = false; 
      
      //6. Removes the highlighting.
      highlightArray = new int[8][8];
    }
  ...
  /**
   * Scans the entire board for a captured king.
   */
  public void checkKing(){
    //Check for Captured King.
    boolean blnWhiteKing = false;
    boolean blnBlackKing = false;
    
    for(int intX = 0; intX <= 7; intX++){
      for(int intY = 0; intY <= 7; intY++){
        if(pieceArray[intX][intY] != null){
          if(pieceArray[intX][intY].strName.equals("KING")){
            if(pieceArray[intX][intY].strColour.equals("WHITE")){
              blnWhiteKing = true;
            }else if(pieceArray[intX][intY].strColour.equals("BLACK")){
              blnBlackKing = true;
            }
          }
        }
    ...
    if(blnWhiteKing == false && blnGame==false){
      System.out.println("Checkmate! from method");
      blnGame = true;
      pieceArray = new Piece[8][8];
      ssm.sendText("9");
      blnWinner = true;
  ...
  /**
   * Scans the entire board for a cpawn promoted to a queen.
   */
  public void promotedPawn(){
    //Check for Promoted Pawn
    for(int intX = 0; intX <= 7; intX++){
      //White Pawn
      if(pieceArray[intX][0] != null){
        if(pieceArray[intX][0].strName.equals("PAWN") && pieceArray[intX][0].strColour.equals("WHITE")){
          int intTempOldX = pieceArray[intX][0].intOldX;
          int intTempOldY = pieceArray[intX][0].intOldY;
          pieceArray[intX][0] = new Queen("WHITE");
          pieceArray[intX][0].intOldX = intTempOldX;
          pieceArray[intX][0].intOldY = intTempOldY;
        }   
        //Black Pawn
      }
      if(pieceArray[intX][7] != null){
        if(pieceArray[intX][7].strName.equals("PAWN") && pieceArray[intX][7].strColour.equals("BLACK")){
          int intTempOldX = pieceArray[intX][7].intOldX;
          int intTempOldY = pieceArray[intX][7].intOldY;
          pieceArray[intX][7] = new Queen("BLACK");
          pieceArray[intX][7].intOldX = intTempOldX;
          pieceArray[intX][7].intOldY = intTempOldY;
        }
      }
    }
  }
  ...
  public void loadGame(String strCurrentColour){
    this.strCurrentColour = strCurrentColour;
  ...
  public void startGame(){
    Thread gt = new Thread(globalTimer);
    gt.start();
    blnGame = true;
  }
